...
class NeuroSamaAgent:
    ...
    class ActionExecutor:
        ...
        def execute(self, action_plan: Dict):
            ...
            if action_type == "speak":
                logger.info(f"[{self.parent.name}说]: {action_plan.get('content', '...')}")
                print(f"[{self.parent.name}说]: {action_plan.get('content', '...')}")
            elif action_type == "control":
                logger.info(f"[系统控制]: 执行 {action_plan.get('action')}")
    ...
    def main_loop(self):
        """核心主循环 - 永不停止的思考"""
        logger.info(f"{self.name} 启动... 开始像人一样思考")
    ...
    def handle_user_interaction(self, decision: Dict, perception: Dict):
        """处理用户交互"""
        logger.info(f"[思考中]: 用户需要我的关注，优先级最高")
    ...
    def handle_self_agenda(self, decision: Dict):
        ...
        if intent == "explore":
            ...
            logger.info(f"[思考中]: 我有点好奇，想去{game}看看")
    ...
    def start(self):
        ...
        logger.info(f"{self.name} 主线程已启动")
        return True
    
    def stop(self):
        """停止智能体"""
        self.is_running = False
        logger.info(f"{self.name} 进入休眠...")
...
# 全局智能体实例（单例模式）
_neuro_agent_instance: Optional[NeuroSamaAgent] = None


def get_neuro_agent() -> NeuroSamaAgent:
    """获取Neuro-Sama智能体单例"""
    global _neuro_agent_instance
    if _neuro_agent_instance is None:
        _neuro_agent_instance = NeuroSamaAgent("Neuro-Sama")
    return _neuro_agent_instance


def start_neuro_agent() -> bool:
    """启动Neuro-Sama智能体"""
    agent = get_neuro_agent()
    return agent.start()


def stop_neuro_agent():
    """停止Neuro-Sama智能体"""
    agent = get_neuro_agent()
    agent.stop()


def get_neuro_agent_status() -> Dict[str, Any]:
    """获取Neuro-Sama智能体状态"""
    agent = get_neuro_agent()
